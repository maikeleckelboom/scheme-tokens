...
| `defineTokenLayer` | Define an ordered overlay layer.  | `defineTokenLayer({ id: "app", tokens: { primary: tokenRef("brand.primary") } })` | Layer object with `tokens`.               | Strict layer input.         | Layers do not own graph modes.                 |
