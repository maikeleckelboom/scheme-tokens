...
const output = runPnpm(["pack", "--pack-destination", packDirectory]).trim().split(/\r?\n/u).at(-1);
if (output === undefined) {
...
const manifest = JSON.parse(tar(["-xOf", tarballName, "package/package.json"])) as PackageManifest;
...
if (
  !tar(["-xOf", tarballName, "package/LICENSE-MATERIAL-COLOR-UTILITIES"]).includes("Apache License")
) {
